-- FU-SJ1b Pavas reverse-order POC.
--
-- LOCAL-ONLY INVESTIGATION PACKAGE. Do not apply remotely.
--
-- Step 0 found the staged 0014 Pavas variants are ordered Pavas -> San Jose,
-- even for both ida/vuelta labels. This POC promotes the most promising
-- near-Pavas families with p_reverse_stop_order = true to test whether the
-- existing CTP staging data can solve Cartago/San Jose -> Pavas before doing
-- any source import work.
...
begin
  ...
  if v_existing_ids > 0 then
    raise exception 'FU-SJ1b Pavas POC precondition failed: % target ruta ids already exist', v_existing_ids;
  end if;
  ...
  if v_seed_patterns > 0 then
    raise exception 'FU-SJ1b Pavas POC precondition failed: % seed patterns already exist', v_seed_patterns;
  end if;
  ...
  if v_missing_variants > 0 then
    raise exception 'FU-SJ1b Pavas POC precondition failed: % target variants missing', v_missing_variants;
  end if;
  ...
  if v_missing_inferred > 0 then
    raise exception 'FU-SJ1b Pavas POC precondition failed: % target variants have too few inferred stops',
      v_missing_inferred;
  end if;
end $$;
...
set metadata = p.metadata || jsonb_build_object(
      'san_jose_fusj1b_pavas_reverse_poc_prior_metadata',
      p.metadata
    ),
...
select public.planner_promote_ctp_variant_to_runtime(
  ...
  'PAVAS',
  'ida',
  'SAN JOSE - PAVAS ZONA 1 / IDA REVERSE POC',
  'local',
  0.870,
  18,
  true,
  'san_jose_fusj1b_pavas_reverse_poc_v1'
);
...
select public.planner_promote_ctp_variant_to_runtime(
  ...
  'PAVAS',
  'vuelta',
  'SAN JOSE - PAVAS ZONA 1 / VUELTA REVERSE POC',
  'local',
  0.870,
  18,
  true,
...
select public.planner_promote_ctp_variant_to_runtime(
  ...
  'PAVAS',
  'ida',
  'SAN JOSE - PAVAS ZONA 2 / IDA REVERSE POC',
  'local',
  0.870,
  18,
  true,
...
select public.planner_promote_ctp_variant_to_runtime(
  ...
  'PAVAS',
  'vuelta',
  'SAN JOSE - PAVAS ZONA 2 / VUELTA REVERSE POC',
  'local',
  0.870,
  18,
  true,
...
begin
  ...
  if v_routes <> 4 or v_patterns <> 4 or v_windows <> 24 or v_bindings <> 4 or v_route_points < 100 then
    raise exception 'FU-SJ1b Pavas POC postcondition failed: routes %, patterns %, windows %, bindings %, route_points %',
