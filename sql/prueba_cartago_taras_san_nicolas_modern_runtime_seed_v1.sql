...
with seed_routes as (
  ...
  from (
    values
      (4719::bigint, 'ida'::text, 'preview-ida-4719-taras-san-nicolas-v1'::text, 18::integer)
  ) as routes(ruta_id, sentido, pattern_code, frecuencia_base_min)
),
preview_stops as (
  select
    sr.ruta_id,
    ...
    0.880::numeric as clasificacion_confianza,
    ...
    (ord - 1)::integer as stop_sequence,
...
select
  ...
  'importacion',
  jsonb_build_object(
    'seed_source', 'preview_cartago_taras_san_nicolas_modern_v1',
...
where rp.ruta_id = 4719
  and rp.fuente = 'importacion'
  and coalesce(rp.metadata ->> 'seed_source', '') = 'preview_cartago_taras_san_nicolas_modern_v1';
...
join lateral (
  values
    ...
    ('habil'::text, '16:00'::time, '20:30'::time, 18::integer),
    ('sabado'::text, '06:00'::time, '20:00'::time, 26::integer),
    ('domingo'::text, '07:00'::time, '19:00'::time, 34::integer),
    ('feriado'::text, '07:00'::time, '19:00'::time, 34::integer)
) as sw(dia_tipo, hora_inicio, hora_fin, frecuencia_promedio_min)
  on true;

with transfer_points as (
  select
    from_stop.id as from_id,
    to_stop.id as to_id,
    round(st_distance(from_stop.geo, to_stop.geo))::integer as distance_m
  from public.paradas source_hint
  join public.paradas from_stop
    on from_stop.activo = true
   and (
     from_stop.id = source_hint.id
     or st_dwithin(from_stop.geo, source_hint.geo, 90)
   )
  join public.paradas to_stop
    on to_stop.id = -13676
   and to_stop.activo = true
  where source_hint.id = -18053
    and source_hint.activo = true
    and round(st_distance(from_stop.geo, to_stop.geo))::integer <= 450
)
