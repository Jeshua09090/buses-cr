-- Rollback for FU-SJ3 Desamparados runtime promotion.
--
-- Removes only the rows created by
-- san_jose_fusj3_desamparados_runtime_promotion_v1.sql.
...
begin
  ...
  if v_routes <> 1 or v_patterns <> 1 or v_windows <> 6 or v_bindings <> 1 then
    raise exception 'FU-SJ3 Desamparados rollback precondition failed: routes %, patterns %, windows %, bindings %',
...
set metadata = p.metadata->'san_jose_fusj3_desamparados_prior_metadata',
    updated_at = timezone('utc'::text, now())
where p.id < 0
  and p.metadata ? 'san_jose_fusj3_desamparados_prior_metadata'
  and p.metadata->>'seed_source' = 'san_jose_fusj3_desamparados_runtime_promotion_v1';
...
begin
  ...
  if v_routes <> 0
    ...
    raise exception 'FU-SJ3 Desamparados rollback postcondition failed: routes %, patterns %, windows %, bindings %, route_points %, unreferenced_stops %',
