-- Rollback for local-only TEC ITCR POC.
-- Removes only rows created by cartago_tec_itcr_poc_v1.sql.
...
begin
  ...
  if v_routes <> 2 or v_patterns <> 2 or v_windows <> 12 or v_bindings <> 2 then
    raise exception 'TEC ITCR POC rollback precondition failed: routes %, patterns %, windows %, bindings %',
...
set metadata = p.metadata->'cartago_tec_itcr_poc_prior_metadata',
    updated_at = timezone('utc'::text, now())
where p.id < 0
  and p.metadata ? 'cartago_tec_itcr_poc_prior_metadata'
  and p.metadata->>'seed_source' = 'cartago_tec_itcr_poc_v1';
...
begin
  ...
  if v_routes <> 0
    ...
    raise exception 'TEC ITCR POC rollback postcondition failed: routes %, patterns %, windows %, bindings %, route_points %, unreferenced_stops %',
