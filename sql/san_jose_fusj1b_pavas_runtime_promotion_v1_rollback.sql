-- Rollback for FU-SJ1b Pavas reverse-order runtime promotion.
--
-- Removes only the rows created by
-- san_jose_fusj1b_pavas_runtime_promotion_v1.sql.
...
begin
  ...
  if v_routes <> 4 or v_patterns <> 4 or v_windows <> 24 or v_bindings <> 4 then
    raise exception 'FU-SJ1b Pavas rollback precondition failed: routes %, patterns %, windows %, bindings %',
...
set metadata = p.metadata->'san_jose_fusj1b_pavas_prior_metadata',
    updated_at = timezone('utc'::text, now())
where p.id < 0
  and p.metadata ? 'san_jose_fusj1b_pavas_prior_metadata'
  and p.metadata->>'seed_source' = 'san_jose_fusj1b_pavas_runtime_promotion_v1';
...
begin
  ...
  if v_routes <> 0
    ...
    raise exception 'FU-SJ1b Pavas rollback postcondition failed: routes %, patterns %, windows %, bindings %, route_points %, unreferenced_stops %',
