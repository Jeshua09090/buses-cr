-- Rollback for FU-SJ3/FU-SJ4 south and northeast local-only POC.
--
...
begin
  ...
  if v_routes <> 3 or v_patterns <> 3 or v_windows <> 18 or v_bindings <> 3 then
    raise exception 'FU-SJ3 south/northeast POC rollback precondition failed: routes %, patterns %, windows %, bindings %',
...
set metadata = p.metadata->'san_jose_fusj3_south_northeast_poc_prior_metadata',
    updated_at = timezone('utc'::text, now())
where p.id < 0
  and p.metadata ? 'san_jose_fusj3_south_northeast_poc_prior_metadata'
  and p.metadata->>'seed_source' = 'san_jose_fusj3_south_northeast_poc_v1';
...
begin
  ...
  from public.planner_ctp_preview_route_bindings
  where metadata->>'seed_source' = 'san_jose_fusj3_south_northeast_poc_v1'
    or ruta_id in (17230, 17231, 17232);
  ...
  if v_routes <> 0
    ...
    raise exception 'FU-SJ3 south/northeast POC rollback postcondition failed: routes %, patterns %, windows %, bindings %, route_points %, unreferenced_stops %',
