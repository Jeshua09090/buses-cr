...
with terminal_stop as (
  select
    ...
    st_y(st_endpoint(st_geometryn(st_transform(rv.geom, 4326), greatest(1, st_numgeometries(st_transform(rv.geom, 4326)))))) as lat,
    st_x(st_endpoint(st_geometryn(st_transform(rv.geom, 4326), greatest(1, st_numgeometries(st_transform(rv.geom, 4326)))))) as lng,
    coalesce(s.description_raw, 'TERMINAL SANATORIO DE DURAN') as nombre
  from public.staging_ctp_official_stops s
  join public.staging_ctp_official_route_variants rv
    on rv.source_id = 924
  where s.source_id = 13696
)
...
select
  ...
  'importacion',
  jsonb_build_object(
    'seed_source', 'preview_cartago_oreamuno_irazu_modern_v1',
    'seed_kind', 'endpoint_terminal_stop',
    'official_stop_source_id', 13696
  ),
  timezone('utc', now()),
  timezone('utc', now())
from terminal_stop ts
...
set parada_inicial_id = ps.parada_inicial_id,
    ...
    metadata = coalesce(rp.metadata, '{}'::jsonb) || jsonb_build_object(
      'endpoint_terminal_fix', 'prueba_cartago_oreamuno_irazu_endpoint_terminal_runtime_fix_v1',
      'endpoint_terminal_stop_id', -13696
    ),
    updated_at = timezone('utc', now())
