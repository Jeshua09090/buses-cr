...
-- Seed moderno para corredores Tejar/Gallito en Prueba.
-- Usa la secuencia preview oficial CTP como fuente determinista de route_patterns
-- y trata de enlazar primero con paradas runtime existentes para conservar
-- transbordos reales en Cartago centro / Gonzaga.

with seed_routes as (
  select *
  from (
    values
      (4190::bigint, 'ida'::text, 'preview-ida-4190-tejar-v1'::text, 'CARTAGO-SAN ISIDRO - EL MOLINO / IDA'::text, 'local'::text, 0.930::numeric, 18::integer),
      (4191::bigint, 'ida'::text, 'preview-ida-4191-tejar-v1'::text, 'CARTAGO-ASUNCION-PITAHAYA-SAN ISIDRO / IDA'::text, 'local'::text, 0.900::numeric, 20::integer),
      (4226::bigint, 'vuelta'::text, 'preview-vuelta-4226-guayabal-v1'::text, 'CARTAGO-HACIENDA VIEJA-TEJAR-GUAYABAL / VUELTA'::text, 'local'::text, 0.860::numeric, 22::integer),
      (4689::bigint, 'ida'::text, 'preview-ida-4689-tejar-v1'::text, 'SAN JOSE-TEJAR / IDA'::text, 'interurbana'::text, 0.940::numeric, 24::integer)
  ) as routes(ruta_id, sentido, pattern_code, pattern_name, categoria_operativa, clasificacion_confianza, frecuencia_base_min)
),
preview_stops as (
  select
    sr.ruta_id,
...
select
  ...
  'importacion',
  jsonb_build_object(
    'seed_source', 'preview_tejar_modern_v1',
...
where rp.ruta_id in (4190, 4191, 4226, 4689)
  and rp.fuente = 'importacion'
  and coalesce(rp.metadata ->> 'seed_source', '') = 'preview_tejar_modern_v1';
...
with pattern_targets as (
  ...
  join (
    values
      (4190::bigint, 18::integer),
      (4191::bigint, 20::integer),
      (4226::bigint, 22::integer),
      (4689::bigint, 24::integer)
...
join lateral (
  values
    ('habil'::text, '05:00'::time, '09:00'::time, greatest(10, pt.frecuencia_base_min)),
    ('habil'::text, '09:00'::time, '16:00'::time, greatest(12, pt.frecuencia_base_min + 4)),
    ('habil'::text, '16:00'::time, '20:30'::time, greatest(10, pt.frecuencia_base_min)),
    ('sabado'::text, '06:00'::time, '20:00'::time, greatest(12, pt.frecuencia_base_min + 4)),
    ('domingo'::text, '07:00'::time, '19:00'::time, greatest(14, pt.frecuencia_base_min + 6)),
    ('feriado'::text, '07:00'::time, '19:00'::time, greatest(14, pt.frecuencia_base_min + 6))
