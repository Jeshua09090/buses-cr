...
-- so this POC promotes both variants with p_reverse_stop_order = true to test
-- Cartago/San Jose -> Escazu downstream drops.
--
-- Local proof only. Do not apply remotely unless the POC validates and a
-- separate remote package is prepared.
...
begin
  ...
  if v_existing_ids > 0 then
    raise exception 'FU-SJ4 Escazu POC precondition failed: % target ruta ids already exist', v_existing_ids;
  end if;

  select count(*)
  into v_seed_patterns
  from public.route_patterns
  where metadata->>'seed_source' = 'san_jose_fusj4_escazu_runtime_poc_v1';

  if v_seed_patterns > 0 then
    raise exception 'FU-SJ4 Escazu POC precondition failed: % seed patterns already exist', v_seed_patterns;
  end if;
  ...
  if v_missing_variants > 0 then
    raise exception 'FU-SJ4 Escazu POC precondition failed: % target variants missing', v_missing_variants;
  end if;
  ...
  if v_missing_inferred > 0 then
    raise exception 'FU-SJ4 Escazu POC precondition failed: % target variants have too few inferred stops',
      v_missing_inferred;
  end if;
end $$;
...
select public.planner_promote_ctp_variant_to_runtime(
  ...
  20,
  true,
  'san_jose_fusj4_escazu_runtime_poc_v1'
);
...
begin
  ...
  where rp.metadata->>'seed_source' = 'san_jose_fusj4_escazu_runtime_poc_v1'
    and sw.activo;
  ...
  if v_routes <> 2 or v_patterns <> 2 or v_windows <> 12 or v_bindings <> 2 or v_route_points < 70 then
    raise exception 'FU-SJ4 Escazu POC postcondition failed: routes %, patterns %, windows %, bindings %, route_points %',
