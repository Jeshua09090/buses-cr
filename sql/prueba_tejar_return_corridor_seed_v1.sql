...
from (
  values
    (
      ...
      'SAN ISIDRO-CARTAGO POR EL MOLINO'::text,
      'GUARCO'::text,
      'CARTAGO'::text,
      '0328-D-2'::text
    ),
    (
      ...
      '328'::text,
      'AUTOTRANSPORTES EL GUARCO SOCIEDAD ANONIMA'::text,
      'SAN ISIDRO-CARTAGO POR ASUNCION'::text,
      'GUARCO'::text,
      'CARTAGO'::text,
      '0328-E-2'::text
    )
...
where ruta_id in (4290, 4291);

with selected_variants as (
  select 4290::integer as ruta_id, '0328-D-2'::text as variant_code
  union all
  select 4291::integer as ruta_id, '0328-E-2'::text as variant_code
),
points as (
  select
    sv.ruta_id,
