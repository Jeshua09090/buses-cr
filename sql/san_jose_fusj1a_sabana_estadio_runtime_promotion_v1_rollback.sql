-- Rollback for FU-SJ1a: remove only the CTP 0007-B-1 Sabana/Estadio
-- runtime promotion created by
-- san_jose_fusj1a_sabana_estadio_runtime_promotion_v1.sql.
...
begin
  ...
  if v_routes <> 1 or v_patterns <> 1 or v_windows <> 6 or v_bindings <> 1 then
    raise exception 'FU-SJ1a rollback precondition failed: routes %, patterns %, windows %, bindings %',
...
set metadata = p.metadata->'san_jose_fusj1a_prior_metadata',
    updated_at = timezone('utc'::text, now())
where p.id < 0
  and p.metadata ? 'san_jose_fusj1a_prior_metadata'
  and p.metadata->>'seed_source' = 'san_jose_fusj1a_sabana_estadio_runtime_promotion_v1';
...
begin
  ...
  if v_routes <> 0
    ...
    raise exception 'FU-SJ1a rollback postcondition failed: routes %, patterns %, windows %, bindings %, route_points %, unreferenced_stops %',
