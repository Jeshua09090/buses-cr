-- Rollback for FU-SJ4 Escazu local POC.
--
-- Removes only rows created by san_jose_fusj4_escazu_runtime_poc_v1.sql.
...
begin
  ...
  if v_routes <> 2 or v_patterns <> 2 or v_windows <> 12 or v_bindings <> 2 then
    raise exception 'FU-SJ4 Escazu POC rollback precondition failed: routes %, patterns %, windows %, bindings %',
...
where p.id < 0
  ...
  and p.metadata->>'seed_source' = 'san_jose_fusj4_escazu_runtime_poc_v1';
...
begin
  ...
  if v_routes <> 0
    ...
    raise exception 'FU-SJ4 Escazu POC rollback postcondition failed: routes %, patterns %, windows %, bindings %, route_points %, unreferenced_stops %',
