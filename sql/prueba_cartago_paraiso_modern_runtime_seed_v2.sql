...
-- Runtime seed for the 0336 Cartago <-> Paraiso trunk.
-- This promotes the official inferred stop sequence into the modern planner
-- tables consumed by buscar_viajes_0_1_transbordo_v2/v3.
...
from (
  values
    (4360::bigint, 'CARTAGO - PARAISO'::text, 'CARTAGO'::text, 'PARAISO'::text, '0336-A-1'::text),
    (4361::bigint, 'PARAISO - CARTAGO'::text, 'PARAISO'::text, 'CARTAGO'::text, '0336-A-2'::text)
) as seed(ruta_id, nombre_ruta, canton_inicio, canton_final, variant_code)
...
with variant_map as (
  ...
  from (
    values
      (4360::bigint, 'ida'::text, '0336-A-1'::text, 'CARTAGO - PARAISO / IDA'::text, 20::integer),
      (4361::bigint, 'vuelta'::text, '0336-A-2'::text, 'PARAISO - CARTAGO / VUELTA'::text, 20::integer)
  ) as variants(ruta_id, sentido, variant_code, pattern_name, frecuencia_base_min)
),
inferred_stops as (
  select
    vm.ruta_id,
    ...
    rs.stop_source_id,
    rs.suggested_stop_sequence::integer as stop_sequence,
    s.source_identifier,
...
select
  ...
  'importacion',
  jsonb_build_object(
    'seed_source', 'preview_cartago_paraiso_modern_v2',
...
inferred_stops as (
  select
    ...
    vm.sentido,
    vm.variant_code,
    concat('preview-', vm.sentido, '-', vm.ruta_id::text, '-cartago-paraiso-v2') as pattern_code,
    vm.pattern_name,
    vm.frecuencia_base_min,
    rs.stop_source_id,
    rs.suggested_stop_sequence::integer as stop_sequence,
...
mapped_stops as (
  select
    ist.*,
    coalesce(nearby.id, ist.synthetic_parada_id)::bigint as parada_id,
    nearby.id as matched_runtime_parada_id
  from inferred_stops ist
  left join lateral (
    select p.id
    from public.paradas p
    where p.activo = true
      and p.id > 0
      and st_dwithin(p.geo, ist.stop_geo, 90)
    order by st_distance(p.geo, ist.stop_geo) asc, p.id asc
    limit 1
  ) nearby on true
),
segmented as (
...
on conflict (pattern_id, stop_sequence) do update
...
using public.route_patterns rp
where sw.pattern_id = rp.id
  and rp.ruta_id in (4360, 4361)
  and coalesce(rp.metadata ->> 'seed_source', '') = 'preview_cartago_paraiso_modern_v2';
