...
begin
  ...
  if v_coords is null or jsonb_typeof(v_coords) <> 'array' then
    raise exception 'El borrador % no tiene una LineString valida para aplicar', p_draft_id;
  end if;
...
comment on function public.aplicar_route_change_draft(bigint) is
  'Aplica un borrador de cambio de trazado sobre ruta_puntos usando una traza grabada o una edicion manual.';
